...
void sig_chld(int singno){
        ...
        while ( (pid = waitpid(-1, &stat, WNOHANG)) > 0)
                printf("Equipment %d disconnected\n", pid);
        return;
}
...
int main(){
        ...
        *shm = 0;
        ...
        if ((maxShm = shmat(shmidVotage, NULL, 0)) == (int *) -1) {
            perror("shmat");
            exit(1);
        }
        *maxShm = 0;
                ...
                if((pid=fork()) == 0) {
                        ...
                        while ((n = recv(connectSock, request, MAXLINE,0)) > 0)  {
                                ...
                                if(strcmp(cmd.code,"STOP") == 0){
                                        ...
                                        *maxShm = *maxShm - currentVoltage - dist;
                                        currentVoltage = 0;
                                        ...
                                        if (atoi(cmd.params[2]) > currentVoltage) {
                                                dist = dist + atoi(cmd.params[2]) - currentVoltage;
                                        } else {
                                                dist = dist - atoi(cmd.params[2]) + currentVoltage;
                                        }
                                        *shm = *shm - currentVoltage;
